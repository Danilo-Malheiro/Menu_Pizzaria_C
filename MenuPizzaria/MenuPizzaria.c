...
#include <time.h>   // para usar time(), localtime() e struct tm
int main() {
    ...
    char nomeArquivo[100]; // data e hora do arquivo .txt
    ...
    // ---- Salvando o pedido em um arquivo novo ----
    FILE *arquivo;

    time_t agora = time(NULL); // Cria um nome único para o arquivo com base na data e hora atual
    struct tm *t = localtime(&agora);
    sprintf(nomeArquivo, "Pedido_%04d%02d%02d_%02d%02d%02d.txt",
            t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
            t->tm_hour, t->tm_min, t->tm_sec);

    arquivo = fopen(nomeArquivo, "w"); 		// Abre o arquivo em modo "w" (cria novo e apaga se já existir)
    ...
    fprintf(arquivo, "---- Resumo do Pedido ----\n");  // Escreve o conteúdo do pedido no arquivo
    for (int i = 0; i < qtdItens; i++) {
